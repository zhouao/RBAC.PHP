...
class authorizator {
	...
	public static function get_new($session_id, $storage){
		...
		//$certifiable = new certifiable($session);
		return $authorizator;
	}

    public function is_allowed($path_ids, $operation_id = 0){
        if($path_ids!=null){
            $certifiable_item = $this->certifiable->get($path_ids);
            $permission = $certifiable_item->permissions[$operation_id];
            //print_r($certifiable_item);
